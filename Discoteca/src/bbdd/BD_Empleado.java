	...
	public Empleado buscarEmpleado(String dni, String contraseña){
		...
		try{
			...
			while ( reg.next()){
				...
				if(!(reg.getString("DNI_EMPLEADO").equalsIgnoreCase(dni) && reg.getString("CONTRASEÑA").equalsIgnoreCase(contraseña))) {
					return null;
				}
				...
				if(reg.getString("OFICIO").equalsIgnoreCase("admin")) 
					retorno = new Admin(reg.getString("DNI_EMPLEADO"),reg.getString("NOMBRE"),reg.getString("APELLIDO"),reg.getString("OFICIO"),fBuena,reg.getString("CONTRASEÑA"));
				
				
				
				retorno = new Empleado(reg.getString("DNI_EMPLEADO"),reg.getString("NOMBRE"),reg.getString("APELLIDO"),reg.getString("OFICIO"),fBuena,reg.getString("CONTRASEÑA"));
